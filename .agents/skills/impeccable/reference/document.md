...
If a `DESIGN.md` already exists, inspect and preserve its still-valid decisions. Honor any refresh, merge, or replacement scope the user already specified. Otherwise update the requested parts without overwriting unrelated content; ask only when a material conflict cannot be resolved from the request and repository.
