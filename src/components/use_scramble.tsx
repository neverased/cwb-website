...
const getLineTone = (line: string) => {
    ...
    if (/authorization|monitored|patching|loading|rendering|syncing/i.test(line)) {
...
export const TerminalLoader = ({ texts }: TerminalLoaderProps) => {
    const [index, setIndex] = useState(0);
    const timeoutRef = useRef<number | null>(null);
    const currentText = texts[index] ?? "";
    const progress = Math.max(6, Math.round(((index + 1) / texts.length) * 100));
    const stage = STAGES.find(({ limit }) => progress <= limit) ?? STAGES[STAGES.length - 1];
    ...
    const scheduleAdvance = (delay: number) => {
        ...
        timeoutRef.current = window.setTimeout(() => {
            setIndex((currentIndex) => (currentIndex >= texts.length - 1 ? 0 : currentIndex + 1));
            timeoutRef.current = null;
        }, delay);
    };
    ...
    const { ref } = useScramble({
        ...
        onAnimationEnd: () => {
            ...
            scheduleAdvance(index >= texts.length - 1 ? 1500 : 120);
        },
    });
        ...
        <div className={styles.shell}>
            ...
            <pre className={styles.screen} aria-live="polite">
                {texts.slice(0, index).map((line, lineIndex) => (
                    <div
                        key={`${line}-${lineIndex}`}
                        className={[styles.line, getLineTone(line)].filter(Boolean).join(" ")}
                    >
                        {line || "\u00A0"}
                    </div>
                ))}
                <div
                    className={[styles.line, styles.activeLine, getLineTone(currentText)].filter(Boolean).join(" ")}
                >
                    {currentText.trim() ? (
                        <span ref={ref} />
                    ) : (
